...
const UserDashboard = () => {
    ...
    const calculateTax = useCallback((salary) => {
        if (salary < 1000000) {
            return salary * 0.10;
    ...
    const calculateBasicSalary = useCallback((salary) => {
        if (salary < 1000000) {
            return salary * 0.25;
        } else if (salary >= 1000000 && salary <= 2000000) {
            return salary * 0.5;
    ...
    const handleSubmit = useCallback(async (values, { setSubmitting }) => {
        try {
            ...
            setIsEditMode(false);
            getUserData(); // Fetch updated user data after successful update
        } catch (error) {
